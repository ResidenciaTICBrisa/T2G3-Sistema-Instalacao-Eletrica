...
class AddAtmosphericDischarge extends StatefulWidget {
  ...
  @override
  _AddAtmosphericDischargeScreenState createState() =>
      _AddAtmosphericDischargeScreenState();
}

class _AddAtmosphericDischargeScreenState
    extends State<AddAtmosphericDischarge> {
  EquipmentService equipmentService = EquipmentService();
  AtmosphericEquipmentService atmosphericService =
  ...
  AtmosphericResponseModel? atmosphericResponseModel;

  @override
  void initState() {
    super.initState();
    _fetchEquipmentCategory();
    if (widget.isEdit && widget.dischargeId != null) {
      _initializeData(widget.dischargeId!);
    }
  }

  Future<void> _initializeData(int dischargeId) async {
    try {
      await _fetchAtmosphericEquipment(dischargeId);

      if (atmosphericResponseModel != null) {
        setState(() {
          equipmentId = atmosphericResponseModel!.equipment;
          _quantity.text = atmosphericResponseModel!.quantity.toString();
          _observationsController.text =
              atmosphericResponseModel!.observation ?? '';
        });

        _fetchEquipmentDetails(atmosphericResponseModel!.equipment);
        _fetchExistingPhotos(atmosphericResponseModel!.equipment);
      }
    } catch (error) {
      print('Error: $error');
    }
  ...
  Future<void> _fetchAtmosphericEquipment(int dischargeId) async {
  ...
  void _updateEquipment() async {
    ...
    if (typeUpdateSuccess) {
      ...
      bool atmosphericUpdateSuccess = await atmosphericService
          .updateAtmospheric(widget.dischargeId!, atmosphericModel);

      if (atmosphericUpdateSuccess) {
      ...
      child: DropdownButton<String>(
        ...
        items: items.map<DropdownMenuItem<String>>((Map<String, Object> value) {
          return DropdownMenuItem<String>(
            value: value['name'] as String,
            enabled:
                value['name'] != 'Selecione o tipo de descarga atmosférica',
            child: Text(
              value['name'] as String,
              style: TextStyle(
                color:
