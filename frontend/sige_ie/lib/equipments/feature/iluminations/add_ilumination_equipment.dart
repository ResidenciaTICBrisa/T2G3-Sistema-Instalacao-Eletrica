...
class AddIluminationEquipmentScreen extends StatefulWidget {
  ...
  @override
  _AddIluminationEquipmentScreenState createState() =>
      _AddIluminationEquipmentScreenState();
}

class _AddIluminationEquipmentScreenState
    extends State<AddIluminationEquipmentScreen> {
  EquipmentService equipmentService = EquipmentService();
  IluminationEquipmentService iluminationService =
      IluminationEquipmentService();
  ...
  final TextEditingController _tecnologyController = TextEditingController();
  ...
  IluminationResponseModel? iluminationResponseModel;

  @override
  void initState() {
    super.initState();
    _fetchEquipmentCategory();
    if (widget.isEdit && widget.iluminationId != null) {
      _initializeData(widget.iluminationId!);
    }
  }

  Future<void> _initializeData(int iluminationId) async {
    try {
      await _fetchIluminationEquipment(iluminationId);

      if (iluminationResponseModel != null) {
        setState(() {
          equipmentId = iluminationResponseModel!.equipment;
          _quantity.text = iluminationResponseModel!.quantity.toString();
          _power.text = iluminationResponseModel!.power.toString();
          _formatController.text = iluminationResponseModel!.format;
          _tecnologyController.text = iluminationResponseModel!.tecnology;
          _observationsController.text =
              iluminationResponseModel!.observation ?? '';
        });

        _fetchEquipmentDetails(iluminationResponseModel!.equipment);
        _fetchExistingPhotos(iluminationResponseModel!.equipment);
      }
    } catch (error) {
      print('Error: $error');
    }
  ...
  Future<void> _fetchIluminationEquipment(int iluminationId) async {
    iluminationResponseModel =
        await iluminationService.getIluminationById(iluminationId);
  }
  ...
  void _updateEquipment() async {
    ...
    if (typeUpdateSuccess) {
      final IluminationRequestModel iluminationModel = IluminationRequestModel(
        ...
        power: int.tryParse(_power.text),
        observation: _observationsController.text.isNotEmpty
            ? _observationsController.text
            : null,
        format:
            _formatController.text.isNotEmpty ? _formatController.text : null,
        tecnology: _tecnologyController.text.isNotEmpty
            ? _tecnologyController.text
            : null,
      );

      print('Ilumination Model: ${iluminationModel.toJson()}');

      bool iluminationUpdateSuccess = await iluminationService
          .updateIlumination(widget.iluminationId!, iluminationModel);

      if (iluminationUpdateSuccess) {
  ...
  Future<void> _registerPersonalEquipmentType() async {
    ...
    if (id != -1) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content:
              Text('Tipo de equipamento de iluminação registrado com sucesso.'),
          backgroundColor: Colors.green,
        ),
      );
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content:
              Text('Falha ao registrar o tipo de equipamento de iluminação.'),
          backgroundColor: Colors.red,
        ),
      );
    }
  ...
  void _showConfirmationDialog() {
    if (_quantity.text.isEmpty ||
        ...
        _tecnologyController.text.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          ...
          content: SingleChildScrollView(
            child: ListBody(
              children: <Widget>[
                ...
                const Text('Formato:',
                    style: TextStyle(fontWeight: FontWeight.bold)),
                Text(_formatController.text),
                const SizedBox(height: 10),
                const Text('Tecnologia:',
                    style: TextStyle(fontWeight: FontWeight.bold)),
                Text(_tecnologyController.text),
                const SizedBox(height: 10),
              ...
              child: Column(
                ...
                children: <Widget>[
                    ...
                    children: [
                      Expanded(
                        ...
                        child: _buildStyledDropdown(
                          ...
                          value: _selectedType,
                          onChanged: (newValue) {
                            if (newValue != 'Selecione o tipo de iluminação') {
                              setState(() {
                  ...
                  Container(
                    ...
                    child: TextField(
                      controller: _tecnologyController,
                      keyboardType: TextInputType.text,
                      decoration: const InputDecoration(
                        border: InputBorder.none,
                        contentPadding:
    ...
    return Container(
      ...
      child: DropdownButton<String>(
        ...
        items: items.map<DropdownMenuItem<String>>((Map<String, Object> value) {
          return DropdownMenuItem<String>(
            value: value['name'] as String,
            enabled: value['name'] != 'Selecione o tipo de iluminação',
            child: Text(
              value['name'] as String,
              style: TextStyle(
                color: value['name'] == 'Selecione o tipo de iluminação'
                    ? Colors.grey
                    : Colors.black,
              ),
            ),
